...
MACRO_CONFIG_INT(SvQuic, sv_quic, 0, 0, 1, CFGFLAG_SERVER, "Open a QUIC endpoint for secure connections, which allow clients to log in to their account (changing requires restart)")
MACRO_CONFIG_INT(SvQuicPort, sv_quic_port, 0, 0, 65535, CFGFLAG_SERVER, "Port to use for the QUIC endpoint (0 = automatically pick a free port; the port is advertised via the server browser) (changing requires restart)")
MACRO_CONFIG_STR(SvAccountServer, sv_account_server, 128, "https://pg.ddnet.org:5555/", CFGFLAG_SERVER, "URL of the account server whose account certificates are accepted, empty to disable accounts (changing requires restart)")
